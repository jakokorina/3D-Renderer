...
glm::mat4 Camera::getCameraTransformationMatrix() const {
    glm::vec3 viewUpVector = direction;
    viewUpVector.y += 1;
    std::vector<glm::vec3> orthonormalCameraBasis(3);
    orthonormalCameraBasis[0] = direction;
    orthonormalCameraBasis[1] = glm::cross(direction, viewUpVector);
    orthonormalCameraBasis[2] = glm::cross(orthonormalCameraBasis[1], direction);
